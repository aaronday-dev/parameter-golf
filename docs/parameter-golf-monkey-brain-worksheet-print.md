---
title: "Parameter Golf Monkey Brain Worksheet"
date: "2026-03-22"
...
- boundary-aware late skip gate -> `2.62201427`
- parameter-neutral `relu^2`-gated GLU -> `2.66081811`
...
- "this mechanism is firing in the wrong place or on too many things"

Bad monkey-brain input:
...
## Current Family Landscape

```text
                           CURRENT FAMILY LANDSCAPE

LIVE ANCHOR
  Sequential higher-capacity control
    best full:   2.35796063
    smoke ctrl:  2.61172375

BEATEN / DOWNRANKED
  Shared-core / mirror / directional C
    useful historically, beaten by plain sequential

  Nearby width/depth pokes
    512 -> 528 lost
    extra mirrored depth lost

  Global control tensors
    damping / attractor / orbit / stabilize
    mostly local losers

  Boundary-aware late skip gate
    killed at 2.62201427

  Parameter-neutral gated MLP
    killed at 2.66081811

STILL LIVE AS FAMILIES
  Salience-conditioned late correction
  Single-tensor quantization-sensitive treatment
  Boundary-aware side signals
  Serializer-aware / codec-aware exceptions
```
...
```text
START FROM ONE LIVE OR DEAD FAMILY

  Modulate scope:
    global -> sparse subset

  Modulate carrier:
    token path -> tensor path -> artifact path

  Modulate byte type:
    float controls -> quantized matrices -> codec exception

  Modulate timing:
    every layer -> late only -> serialize only

  Modulate selector:
    all tokens -> rare token class -> one tensor family

EXAMPLE OUTPUTS
  rare-token late correction
  one-tensor special treatment
  quantizable structural change instead of float controls
  serializer-side intervention
  boundary / salience / token-class conditioned family
```
...
- the saved artifact is now "integer codes + scales"

```text
FLOAT WEIGHT ROW
    |
    v
choose row scale / clipping range
    |
    v
divide by scale and round to int8 bins
    |
    +--> store int8 codes in [-127, 127]
    |
    +--> store scale metadata
    |
    v
dequantize at eval using codes * scale
```
...
- tiny float control tensors are dangerous because they skip the cheap big-matrix pattern
...
```text
int8 payload bytes + scale bytes
            |
            v
      byte stream
            |
            v
dictionary matches repeated byte patterns
            |
            v
range coding packs predictable symbols tightly
            |
            v
      compressed artifact
```
...
train float weights
      |
      v
INT8 turns geometry into buckets
      |
      v
artifact becomes bytes + scales
      |
      v
LZMA turns repeated byte patterns into savings
      |
      v
exact post-roundtrip val_bpb
```
...
Then build the patch from those three things.
