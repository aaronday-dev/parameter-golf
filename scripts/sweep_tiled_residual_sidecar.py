...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Offline sweep for a column-tiled low-rank residual sidecar on one sacred tensor. "
            "Builds a normal int8 artifact, then restores local residual structure tile-by-tile."
        )
    )
    ...
    parser.add_argument(
        "--candidate-only",
        action="store_true",
        help="Skip baseline evals and only exact-evaluate the requested tiled candidates.",
    )
    parser.add_argument(
        "--target-tensor",
        default="blocks.0.mlp.proj.weight",
        help="Exact tensor name to carry with a tiled low-rank residual sidecar.",
    )
    parser.add_argument(
        "--tile-cols-list",
        default="1024,512",
        help="Comma-separated column tile widths to test.",
    )
    parser.add_argument(
        "--ranks",
        default="16,24,32,40,48,64",
        help="Comma-separated per-tile ranks to test.",
    )
...
def build_tiled_sidecar(residual: np.ndarray, tile_cols: int, rank: int) -> tuple[list[dict[str, object]], int]:
    rows, cols = residual.shape
    if cols % tile_cols != 0:
        raise ValueError(f"tile_cols={tile_cols} does not divide tensor width {cols}")
    tiles: list[dict[str, object]] = []
    raw_bytes = 0
    for col_start in range(0, cols, tile_cols):
        col_end = col_start + tile_cols
        tile_residual = residual[:, col_start:col_end]
        left, right = trunc_svd_sidecar(tile_residual, rank)
        tiles.append(
            {
                "row_start": 0,
                "row_end": rows,
        ...
        raw_bytes += int(left.nbytes + right.nbytes)
    return tiles, raw_bytes
...
def main() -> None:
    ns = parse_args()
    tile_cols_list = parse_int_list(ns.tile_cols_list)
    ranks = parse_int_list(ns.ranks)
    ...
    if not ns.candidate_only:
        keepf_obj, keepf_stats = build_quant_obj(float_flat, ns.target_tensor, keep_float_target=True, tiled_sidecar=None)
    ...
    plain_obj, plain_stats = build_quant_obj(float_flat, ns.target_tensor, keep_float_target=False, tiled_sidecar=None)
    ...
    target_dequant = target_q.astype(np.float32) * target_scale[:, None] if target_scale.ndim > 0 else target_q.astype(np.float32) * float(target_scale)
    ...
    for tile_cols in tile_cols_list:
        print(f"Testing tile_cols {tile_cols}")
        for rank in ranks:
            print(f"Testing tile_cols {tile_cols} rank {rank}")
            tiled_sidecar, sidecar_raw_bytes = build_tiled_sidecar(residual, tile_cols, rank)
            quant_obj, stats = build_quant_obj(
                float_flat,
                ns.target_tensor,
                keep_float_target=False,
                tiled_sidecar=tiled_sidecar,
            )
            ...
            _, val_bpb = evaluate_state(
                ...
                has_leading_space_lut,
                is_boundary_token_lut,
                log_prefix=f"[tile {tile_cols} rank {rank}] ",
            )
            row = {
                "tile_cols": tile_cols,
                "rank": rank,
                ...
                "sidecar_raw_bytes": sidecar_raw_bytes,
    ...
    for row in rows[:10]:
        ...
        plain_text = "n/a" if row["bpb_delta_vs_plain"] is None else f"{row['bpb_delta_vs_plain']:+.8f}"
        print(
            f"tile_cols={row['tile_cols']} rank={row['rank']} artifact={row['artifact_bytes']:,} "
